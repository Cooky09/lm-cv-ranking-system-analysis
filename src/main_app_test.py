...
from process_cv import process_cvs_multiprocess, calculate_similarity_scores, calculate_weighted_similarity_scores, process_resume
...
celery = Celery("main_app_test", broker=app.config['broker_url'])
...
def analyze_cvs_task(self, file_paths, sample_jd):
    ...
    for file_path in file_paths:
        ...
        try:
            ...
            cv_file, text, name, designation, experience, education, skills = process_resume(file_path)
            context_score = calculate_weighted_similarity_scores(sample_jd, experience, education, skills)
...
@celery.task(bind=True)
def analyze_cvs_in_folder(self, folder_path, sample_jd):
    logging.info(f"Starting CV analysis task for folder: {folder_path}")
    results = []

    try:
        file_names, cv_texts, names, designations, experiences, educations, skills_list = process_cvs_multiprocess(folder_path)

        assert len(file_names) == len(cv_texts) == len(names) == len(designations) == len(experiences) == len(educations) == len(skills_list), "Mismatched lengths in CV data"

        if not file_names:
            logging.warning("No valid CV files found.")
            return {'error': "No valid CV files found."}
        
        print("Sample JD: ", sample_jd)

        # calculate similarity
        similarity_scores = calculate_weighted_similarity_scores(sample_jd, experiences, educations, skills_list)

        # put everything in results list
        results = []
        for i in range(len(file_names)):
            context_score_percentage = f"{similarity_scores[i] * 100:.2f}%"  # change to percentage
            results.append({
                "Resume Name": file_names[i],
                "Name": names[i],
                "Designation": designations[i],
                "Experience": experiences[i],
                "Education": educations[i],
                "Skills": skills_list[i],
                "Context Score": context_score_percentage
            })
    
    except Exception as e:
        logging.error(f"Error processing file in folder {folder_path}: {e}")
...
def analyze_cvs():
    ...
    task = analyze_cvs_in_folder.apply_async(args=[file_paths, sample_jd])
    return jsonify({"task_id": task.id}), 202

@app.route('/api/analyze-cvs/<task_id>', methods=['GET'])
def get_analysis_result(task_id):
    logging.info(f"Checking status of task {task_id}")
    task = analyze_cvs_in_folder.AsyncResult(task_id)
